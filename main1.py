...
class WidgetCamera(QWidget):
    ...
    def open_camera(self, use_camera, video):
        """打开摄像头，成功打开返回True"""
        cam = GLOBAL.config['netstreamvedio']  # 默认摄像头
        #print(cam)
        if not use_camera:
            cam = video  # 视频流文件
    ...
    def draw(self, qp):
        ...
        if self.fps > 0:
            ...
            qp.setPen(pen)
            qp.drawText(self.width() - 150, yh, 'FPS: ' + str(round(self.fps, 2)))
        ...
        if self.fps>0:
            ...
            qp.setFont(font)
            qp.setPen(pen)
            qp.drawText(0, yh-20, "there are {0} person".format(person+hat))
            qp.drawText(0, yh, "{0} people did not wear safety helmets".format(person))
        #print(person,hat)
