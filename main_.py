...
class WidgetCamera(QWidget):
    ...
    def show_camera(self):
        ...
        while self.opened:
            ...
            pool.apply_async(self.m_read_image,(self.cap1,self.url1,queue))
            pool.apply_async(self.m_read_image,(self.cap2,self.url2,queue))
            pool.apply_async(self.m_read_image,(self.cap3,self.url3,queue))
            pool.apply_async(self.m_read_image,(self.cap4,self.url4,queue))
            self.image1,self.cap1 = queue.get()
            self.image2,self.cap2 = queue.get()
            self.image3,self.cap3 = queue.get()
            self.image4,self.cap4 = queue.get()
            #self.read_images()
            #print(self.num)
    ...
    def m_read_image(self, cap,url,queue):
        ret, image = cap.read()
        if ret:
            if image.shape[2] ==4:
                image = image[:,:,:-1]
