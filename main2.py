...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.camera1 = WidgetCamera('rtsp://admin:mkls1123@192.168.0.65/')
        self.camera2 = WidgetCamera('rtsp://admin:mkls1123@192.168.0.66/')
        self.camera3 = WidgetCamera('rtsp://admin:mkls1123@192.168.0.67/')
        self.camera4 = WidgetCamera('rtsp://admin:mkls1123@192.168.0.68/')        # 摄像头
    ...
    def oc_camera(self):
        ...
        if self.camera1.cap.isOpened():
            ...
            self.t1 = threading.Thread(target=self.muti_thread_detect,args=(self.camera1,))
            self.t2 = threading.Thread(target=self.muti_thread_detect,args=(self.camera2,))
            self.t3 = threading.Thread(target=self.muti_thread_detect,args=(self.camera3,))
            self.t4 = threading.Thread(target=self.muti_thread_detect,args=(self.camera4,))
            self.t1.start()
            self.t2.start()
            self.t3.start()
            self.t4.start()
